...
class E8P12_codebook(nn.Module):
    ...
    def __init__(self, inference=False):
        ...
        if not inference:
            ...
            self.register_buffer(
                "part_abs_map",
                self.round(grid_part.abs(), abs_grid, abs_grid.norm(dim=-1) ** 2)[1],
            )
    ...
    def quantize(self, X, return_idx=True, **kwargs):
        ...
        final_vals = torch.where(
            which.unsqueeze(-1), plus_vals - 1 / 4, minus_vals + 1 / 4
        )
...
class QuantizedE8P12Linear(nn.Module):
    ...
    def cache_WH(
        self, n, m, Qidxs_list, had_left, had_right, K_left, K_right, **kwargs
    ):
