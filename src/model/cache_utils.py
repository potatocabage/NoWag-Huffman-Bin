...
class QuantoQuantizedCache(QuantizedCache):
    ...
    def _quantize(self, tensor, axis):
        scale, zeropoint = self.optimizer(
            tensor, self.qtype.bits, axis, self.q_group_size
        )
        qtensor = AffineQuantizer.apply(
            tensor, self.qtype, axis, self.q_group_size, scale, zeropoint
        )
        return qtensor
...
class StaticCache(Cache):
    ...
    def update(
        ...
        if cache_position is None:
            ...
            try:
                k_out.index_copy_(
                    2, cache_position.to(k_out.device), key_states.to(k_out.device)
                )
                v_out.index_copy_(
                    2, cache_position.to(v_out.device), value_states.to(v_out.device)
                )
...
class EncoderDecoderCache(Cache):
    ...
    def get_seq_length(self, layer_idx: Optional[int] = 0) -> int:
        ...
        if self.self_attention_cache.key_cache == []:
            return 0
        if (
            len(self.self_attention_cache.key_cache) > 1
            and self.self_attention_cache.key_cache[layer_idx] == []
        ):
            return 0
...
class OffloadedStaticCache(StaticCache):
    ...
    def _create_key_value_cache_tensors(
        ...
        key_cache = torch.zeros(
            shape, dtype=self.dtype, device=device, pin_memory=is_cpu_device
        )
